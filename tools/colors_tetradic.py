...
# Write tetradic rows
with open("_data/colors/tetradic.txt", "w", newline="") as f_out:
    instructions = """
...
"""
    ...
    for full_key, rgb in name_to_rgb.items():
        ...
        c1_rgb = rotate_hue(rgb, 90)
        c2_rgb = rotate_hue(rgb, 180)
        c3_rgb = rotate_hue(rgb, 270)

        c1_name, _ = closest_color(c1_rgb, name_to_rgb)
        c2_name, _ = closest_color(c2_rgb, name_to_rgb)
        c3_name, _ = closest_color(c3_rgb, name_to_rgb)

        c1_name = c1_name.split("|")[0]
        c2_name = c2_name.split("|")[0]
        c3_name = c3_name.split("|")[0]

        line = f"{base_name} and {c1_name} <var:color_var_1>, {c2_name} and {c3_name} <var:color_var_2>\n"
        f_out.write(line)
